...
项目根目录提供 [`.env.example`](../.env.example)，复制为 `.env` 后可集中填写本地开发和构建变量。Python 后端入口会先读取根目录 `.env`：其中的非空值覆盖系统环境变量，空值视为未设置并继续使用系统环境变量；两处都为空时，环回模式采用内置默认值，远端模式或其他必需配置会报错，并提示在 `.env` 或系统环境变量中补齐。`.env`、密码、密钥和令牌不要提交。

部署工具的配置边界保持独立：Docker Compose 读取 `deploy/.env`，Workers 本地调试读取 `deploy/worker/.dev.vars`，GitHub Actions 读取仓库 Secrets/Variables；这些文件不会回退到根目录 `.env`。模板中未使用的变量留空即可，程序会跳过并继续按上述优先级查找。

...
async def _do_sign(
    ...
    *,
    ...
    当签到返回极验（Geetest）验证码时，返回短期挑战供人工验证后重试。
    凭据恢复与无挑战的高频风控仍有有界重试，避免反复请求上游。
    ...
    for attempt in range(CAPTCHA_MAX_RETRIES + 1):
        ...
        headers.update(game_cfg.get("extra_headers", {}))
        if verification:
            for key in ("challenge", "validate", "seccode"):
                headers[f"x-rpc-{key}"] = verification[f"geetest_{key}"]
            ...
            return _SignOutcome(
                {
                    ...
                    "status": "风控", "reward": "", "reason": "请完成人工验证后继续签到",
                    "_verification": {
                        "kind": "game", "gt": str(data["gt"]), "challenge": str(data["challenge"]),
                        "gameBiz": next(key for key, value in GAME_CONFIG.items() if value is game_cfg or value == game_cfg),

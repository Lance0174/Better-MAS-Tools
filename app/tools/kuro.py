...
def parse_kuro_credential(raw: str) -> tuple[str, str, str]:
    """兼容旧 Token；短信登录凭据保留同一设备标识供后续请求使用。"""
    value = raw.strip()
    if value.startswith("{"):
        try:
        ...
        return token.strip(), str(data.get("devCode") or device), str(
            data.get("distinctId") or distinct
        )
    if not value or any(char.isspace() for char in value):
        raise ValueError("库街区 Token 格式无效")
    return value, *_kuro_device_identifiers(value)


# 短信接口与请求字段参考 mxyooR/Kuro_login（MIT，文件头致谢保留）。
KURO_CAPTCHA_ID = "3f7e2d848ce0cb7e7d019d621e556ce2"


async def request_kuro_sms(
    *, phone: str, verification: dict[str, str], device: str, distinct: str,
    proxy: str | None = None,
) -> None:
    async with network.client(proxy=proxy, trust_env=False, timeout=20) as client:
        response = await client.post(
            "https://api.kurobbs.com/user/getSmsCode",
            headers=_kuro_request_headers(BBS_HEADERS, "", device, distinct),
            data={"mobile": phone, "geeTestData": json.dumps(verification)},
        )
        _check_kuro_login_response(response, "短信发送")


def _check_kuro_login_response(response: httpx.Response, stage: str) -> dict:
    """登录错误只回显业务码，禁止回显可能包含手机号或 Token 的原始响应。"""
    try:
        payload = response.json()
    except ValueError:
        raise ValueError(f"库街区{stage}响应无效，请稍后重试") from None
    if not isinstance(payload, dict):
        raise ValueError(f"库街区{stage}响应格式无效")
    code = str(payload.get("code", ""))
    if not response.is_success or code != "200":
        safe_code = code if code.isdigit() and len(code) <= 8 else "未知"
        raise ValueError(f"库街区{stage}未完成（code={safe_code}），请检查验证码或稍后重试")
    return payload


async def login_kuro_sms(
    *, phone: str, code: str, device: str, distinct: str, proxy: str | None = None,
) -> str:
    async with network.client(proxy=proxy, trust_env=False, timeout=20) as client:
        response = await client.post(
            "https://api.kurobbs.com/user/sdkLogin",
            headers=_kuro_request_headers(BBS_HEADERS, "", device, distinct),
            data={"mobile": phone, "code": code, "devCode": device, "gameList": ""},
        )
        data = _check_kuro_login_response(response, "登录").get("data")
        if not isinstance(data, dict) or not isinstance(data.get("token"), str) or not data["token"].strip() or not data.get("userId"):
            raise ValueError("库街区登录未取得完整凭据")
        # 只提取正式凭据；手机号、短信码及其他登录响应均不进入存储。
        return json.dumps({
            "token": data["token"].strip(), "userId": str(data["userId"]),
            "devCode": device, "distinctId": distinct,
        }, ensure_ascii=False)
...
async def kuro_sign_in(token: str, proxy: str | None = None) -> list[dict[str, object]]:
    ...
    resolved_proxy = proxy if proxy is not None else network.proxy
    async with network.client(proxy=resolved_proxy, trust_env=False) as client:
        # 获取用户信息
        try:

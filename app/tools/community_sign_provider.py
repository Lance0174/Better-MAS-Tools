...
async def check_community_system_time(*, time_source: _TimeSource = time) -> None:
    ...
    try:
        async with network.client(proxy=network.proxy) as client:
...
def _decorate_provider_run(
    ...
    *,
    ...
    for raw_item in run.results:
        ...
        if isinstance(raw_item.get("_verification"), dict):
            # 短期挑战交由 core 消费，保存签到结果前必须剥离。
            normalized[-1]["_verification"] = raw_item["_verification"]
